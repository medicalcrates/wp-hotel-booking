...
(function ($) {
	...
	$('.child_tooltip').click( function () {
		alert(hotel_booking_i18n.childhelp);
		return false;
	});
	...
	$(document).ready(function () {
		...
		$('form#hb-payment-form').submit(function (e) {
			...
			var email = $('.hb-form-field-input input[name=email]').val();
			var phone = $('.hb-form-field-input input[name=phone]').val();
			var name = $('.hb-form-field-input input[name=first_name]').val() + " " + $('.hb-form-field-input input[name=last_name]').val();
		...
		function newCookie(name,value,days) {
			...
			function toMem(a) {
				var name = document.form.first_name.value() + " " + document.form.last_name.value();
				alert(name);
				/*newCookie('theName', name);     // add a new cookie as shown at left for every
	...
	$('.SendPayForm_Button').click(function() {
